...
def refresh():
	console.print(f"[bold green][*] Returning to main menu in 5 seconds.....")
	time.sleep(5)
...
def add_accounts(key):
	website = console.input(f"[#FFA500][?] Enter the name of the Website: ")
	username = console.input(f"[#FFA500][?] Enter the Username of the Website: ")
...
def edit_accounts(key):
	display_accounts(key)
	try:
		account_id = int(console.input(f"[#FFA500][?] Enter the ID of the account that you want to edit: "))
	except ValueError:
	...
	if not result:
		...
		conn.close()
		return
	website = console.input(f"[#FFA500][?] Enter the New Website Name (Leave Blank to keep '{result[1]}'): ") or result[1]
	username = console.input(f"[#FFA500][?] Enter the New Username (Leave Blank to keep '{result[2]}'): ") or result[2]
...
def delete_accounts(key):
	display_accounts(key)
	try:
		account_id = int(console.input(f"[#FFA500][?] Enter the ID of the account that you want to DELETE: "))
	except ValueError:
		console.print(f"[bold red][!] Invalid ID Entered. Try Again.....")
		return
	confirm = console.input(f"[#FFA500][?] Are you sure you want to delete this account? (y/N): ").lower()
	if confirm != "y":
		console.print(f"[bold green][*] Deletion aborted!")
		return
	conn = sqlite3.connect("VaultPass.db")
	cursor = conn.cursor()
...
def generate_password():
	length = int(console.input(f"[#FFA500][?] Enter the minimum length of the Password: "))
	if length < 16:
...
└─────────────────────────────────────────────────────────────────────────────────────────────────────┘
	""")
	console.print(rf"[bold green]+--------------------------------------------------------------+")
	console.print(rf"[bold green]  VaultPass - Offline Security. Encrypted by Design. Yours Forever.")
	console.print(rf"[bold green]  Created by [bold black]Cursed271")
	console.print(rf"[bold green]+--------------------------------------------------------------+")
...
if __name__ == "__main__":
	...
	create_database()
	while True:
		console.print(rf"[bold green][+] Welcome to VaultPass")
		console.print(rf"[bold green]    [1] Add Accounts")
		console.print(rf"[bold green]    [2] Edit Accounts")
		console.print(rf"[bold green]    [3] Delete Accounts")
		console.print(rf"[bold green]    [4] View Accounts")
		console.print(rf"[bold green]    [5] Password Generator")
		console.print(rf"[bold green]    [6] Exit")
		choice = console.input(rf"[#FFA500][?] Select an Option: ")
		if choice == "1":
			add_accounts(key)
			refresh()
		elif choice == "2":
